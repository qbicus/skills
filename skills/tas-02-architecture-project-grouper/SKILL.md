...
Run the bundled grouper script against an analyzer workspace, then review only the low-confidence groupings or unknowns. Favor explicit evidence from analyzer outputs, allow conservative AI-assisted normalization where analyzer fields are too generic, and leave weak candidates as orphans instead of forcing platform membership.
...
- Treat analyzer `output.json` files as the source of truth.
- Group conservatively.
- Do not force grouping.
- Allow orphan projects.
- Build a normalized per-project view before grouping when analyzer fields are too generic for direct matching.
...
- A normalized auth system or dependency link inferred by the model must be recorded in `decisions.md`.
...
- When analyzer `security` fields contain generic evidence text rather than normalized auth products or protocols, infer conservative auth families from combined analyzer evidence instead of assuming the old field shape still exists.
- When analyzer `integrations` are host-style names or aliases, normalize them against project names, deployable units, and component names before deciding whether they imply cross-project dependencies.
